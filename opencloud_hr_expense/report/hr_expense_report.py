...
class report_expense_expense(report_sxw.rml_parse):

    def set_context(self, objects, data, ids, report_type=None):
        res = super(report_expense_expense, self).set_context(objects, data, ids, report_type=report_type)
        ...
        for expense in hr_expense_obj.browse(self.cr, self.uid, expenses):
            for expense2 in expense.line_ids:
                if expense2.state not in ('cancel'):
                    key = expense.employee_id.name + '-' + expense.currency_id.name + '-' + expense.state
                    if expenses_info.get(key):
                        expenses_info[key]['lines'] += expense2
...
class report_hr_expense_expense(osv.AbstractModel):
    _name = 'report.hr_expense.report_expense'
    _inherit = 'report.abstract_report'
    _template = 'hr_expense.report_expense'
    _wrapped_report_class = report_expense_expense
