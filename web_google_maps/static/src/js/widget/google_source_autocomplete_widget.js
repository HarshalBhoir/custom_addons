odoo.define('web_google_maps.GoogleSourceAutocomplete', function (require) {
    "use strict";
    ...
    var FieldCharGoogleSourceAutocomplete = form_widgets.FieldChar.extend({
        template: 'web_google_maps.FieldGoogleSourceAutocomplete',
        ...
        init: function (field_manager, node) {
            ...
            this.source_autocomplete = false;
            this.component_form = MapViewSourceAutocomplete.GOOGLE_PLACES_COMPONENT_FORM;
            this.fillfields = {
                general: {
                    source: ['source', 'street_number', 'route','administrative_area_level_3', 
                ...
                address: {
                    source_address: ['source', 'street_number', 'route','administrative_area_level_3', 
                    'administrative_area_level_4', 'administrative_area_level_5','locality', 'administrative_area_level_2',
                    'postal_code','administrative_area_level_1','country'],
                }
            };
        ...
        gmaps_initialize: function () {
            var self = this;
            this.source_autocomplete = new google.maps.places.Autocomplete(this.$input.get(0));
            // When the user selects an address from the dropdown, populate the address fields in the form.
            this.source_autocomplete.addListener('place_changed', function () {
                var place = this.getPlace();
                var values = {};
                var requests = [];
                if (place.hasOwnProperty('address_components')) {
                    // Get address
                    var google_address = MapViewSourceAutocomplete.gmaps_populate_address(place, self.fillfields.address, self.fillfields_delimiter);
                    _.extend(values, google_address);
                    // Get place (name, website, phone)
                    var google_place = MapViewSourceAutocomplete.gmaps_populate_places(place, self.fillfields.general);
                    _.extend(values, google_place);
                    // Get place geolocation
                    var google_geolocation = MapViewSourceAutocomplete.gmaps_get_geolocation(place, self.fillfields.geolocation);
        ...
        prepare_value: function (model, field_name, value) {
            return MapViewSourceAutocomplete.odoo_prepare_values(model, field_name, value);
        },
        ...
        destroy_content: function () {
            google.maps.event.clearInstanceListeners(this.source_autocomplete);
            this._super.apply(this, arguments);
        }
    });

    core.form_widget_registry.add('gsource_autocomplete', FieldCharGoogleSourceAutocomplete);

    return FieldCharGoogleSourceAutocomplete;
